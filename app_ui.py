...
st.markdown(
    '<div class="section-note">아래 웨이퍼맵과 SHAP 근거는 선택한 웨이퍼의 <b>실제 기록된 공정값</b>을 '
    "사용합니다 — 사이드바 슬라이더와는 무관하게 항상 고정된 과거 실측 데이터입니다. "
    '슬라이더 조작 결과는 이 아래 "🎛️ What-If 시뮬레이션" 절에서 확인하세요.</div>',
    unsafe_allow_html=True,
)
...
- **SHAP은 인과관계가 아닙니다**: 노광 파장(UV_type)은 초기 분석에서 유의해 보였으나 특정 Lot 교란을
  통제하자 통계·ML 10개 모델 중 9개에서 효과가 사라졌고, 남은 1개(LightGBM)도 개선 폭이 R² +0.0036으로
  실무상 무시할 수준이었습니다. 공정 변경 전 `reports/hypothesis_H1_H7_summary.md`를
  반드시 교차 확인하세요.
