...
- SHAP 기여도는 인과관계가 아니다(4절) — UV_type처럼 Lot 교란을 통제하자 사라진 신호가
  실제로 있었으므로, 공정 변경 전 반드시 해당 가설 리포트를 교차 확인해야 한다.
- 원본 데이터 특성상 웨이퍼맵(die-level 26×26 이미지)의 17%는 파싱 시 절단되어 복구할 수
  없다 — 대시보드에서 해당 웨이퍼 선택 시 이를 명시한다.
